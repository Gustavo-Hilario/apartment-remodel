...
export default function ProductsPage() {
        ...
        try {
            ...
            setProducts((prevProducts) =>
                prevProducts.map((p) => {
                    ...
                    const pId =
                        p.uniqueId || p._id || `${p.room}-${p.originalIndex}`;
                    const productId =
                        product.uniqueId ||
                        product._id ||
                        `${product.room}-${product.originalIndex}`;
    ...
    const getFilteredAndSortedProducts = () => {
        ...
        if (sortBy === 'favorites') {
            ...
            filtered.sort((a, b) => {
                const priceA =
                    (a.unitPrice || a.actualRate || a.budgetRate || 0) *
                    (a.quantity || 0);
                const priceB =
                    (b.unitPrice || b.actualRate || b.budgetRate || 0) *
                    (b.quantity || 0);
